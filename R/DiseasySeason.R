...
DiseasySeason <- R6::R6Class(
  ...
  public = list(
    ...
    use_season_model = function(model_name, dots) {
      ...
      eval(parse(
        text=glue::glue("self$use_{model_name}({dots_to_string})")))
    },
    ...
    use_covid_season_v1 = function(scale = 0.4825524) {
      ...
      max_scale <- 1 - (1 - (1 + exp(-b* ( max(climate_normal$max_temp) - t0)))^(-1)) /
                       (1 - (1 + exp(-b * (min(climate_normal$max_temp) - t0)))^(-1))
      ...
      beta_sweden <- \(a) purrr::partial(private$generalized_logistic_function,
                                   a = a, k = k, b = b, x0 = t0)
    ...
    use_covid_season_v2 = function(scale = 0.5042782) {
      ...
      max_scale <- 1 - (1 - (1 + exp(-b * (max(climate_normal$max_temp) - t0)))^(-1/nu)) /
                       (1 - (1 + exp(-b * (min(climate_normal$max_temp) - t0)))^(-1/nu))
      max_scale <- floor(max_scale * 100) / 100 # Remove the very high end of the scale
      ...
      # Read data from DMI
      temperature <- mg::get_table(private$conn, "mg.dmi", slice_ts = self$slice_ts)
    ...
    describe = function() {
      mg::printr("# Season model ###############################################")
      if (is.null(private$.model_date)) {
        mg::printr("Season model not yet set")
      } else {
        mg::printr(glue::glue("Season model: {attr(private$.model_date, 'name')}"))
        mg::printr(glue::glue("{attr(private$.model_date, 'description')}"))
        if (!is.null(attr(private$.model_date, 'dots'))) {
          mg::printr("Parameters: ",
      ...
      if (is.null(self$reference_date)) {
        mg::printr("Reference date not yet set")
      } else {
        mg::printr(glue::glue("Reference date: {self$reference_date}"))
      }
    }
  ),
