...
DiseasyActivity <- R6::R6Class(
  ...
  public = list(
    ...
    crop_scenario = function(first_date = NULL, last_date = NULL) {
      ...
      # Input checks
      coll <- checkmate::makeAssertCollection()
      checkmate::assert_date(first_date, any.missing = FALSE, len = 1, null.ok = T, add = coll)
      checkmate::assert_date(last_date,  any.missing = FALSE, len = 1, null.ok = T, add = coll)
      checkmate::reportAssertions(coll)
    ...
    get_scenario_contacts = function(age_cuts_lower = NULL, population_1yr = NULL) {
      ...
      # Input checks
      coll <- checkmate::makeAssertCollection()
      checkmate::assert_numeric(age_cuts_lower, any.missing = FALSE, null.ok = TRUE, lower = 0, unique = TRUE, add = coll)
      ...
      for (dd in seq_along(openness)) { #looping over dates
        for (tt in private$activity_types) {
          contacts[[dd]][[tt]] <- private$vector_to_matrix(openness[[dd]][[tt]]) * self$contact_basis$counts[[tt]] # TODO: genWeight
        }
      }
      ...
      if (!is.null(age_cuts_lower)) {
        ...
        # Label the matrix
        dimnames(p) <- list(mg::age_labels(age_cuts_lower), names(self$contact_basis$prop))
    ...
    reset_scenario = function() {
      ...
      private$lg$info("`$scenario_matrix` and `$risk_matrix` reset to zero columns. This means that you have to recreate scenarios.")
    },
