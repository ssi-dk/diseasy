...
#' @export
DiseasyActivity <- R6::R6Class(                                                                                         # nolint: object_name_linter, namespace_linter. We need to suppress namespace_linter until R-CMD-Check works with R6 fully
  ...
  public = list(
    ...
    initialize = function(base_scenario = "closed", activity_units = NULL, contact_basis = NULL, ...) {
      ...
      lgr::without_logging(self$reset_scenario()) # Configure internal matrices to activity_unit dimensions             # nolint: namespace_linter. We need to suppress until R-CMD-Check works with R6 fully
    ...
    get_scenario_openness = function(age_cuts_lower = NULL, weights = NULL) {
      ...
      if (length(scenario_activities) == 0) {
        ...
        if (!is.null(age_cuts_lower)) {
          ...
          proportion <- aggregate(proportion ~ age_group_reference, data = population, FUN = sum)$proportion
