  ...
  public = list(
    ...
    get_results = function(observable, prediction_length, quantiles = NULL, aggregation = NULL) {
      ...
      if (!private$is_cached(hash)) {
        ...
        training_data <- self$get_training_data(observable, aggregation)
        ...
        new_data <- purrr::map(c(-(seq(self$parameters$training_length) - 1), seq(prediction_length)),                  # nolint: infix_spaces_linter
                               ~ dplyr::mutate(prototype_data,
                                               t = .x,
                                               date = max(training_data$date) + lubridate::days(t))) |>
        ...
        prediction <- prediction |>
          ...
          dplyr::mutate(weight = 1,  # All realizations have the same weight
                        model = self$hash) # Add meta information to the data
