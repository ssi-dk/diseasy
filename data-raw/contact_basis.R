if (require(contactdata) && require(countrycode) && require(curl) && require(usethis)) {
  ...
  contact_basis <- common_country_codes |>
    purrr::map(\(country_code) {
      tibble::lst(contacts = NULL, # Allocate for below
  ...
  contact_basis <- purrr::map(contact_basis, \(basis) {
    ...
    N <- basis$demography |>                                                                                            # nolint: object_name_linter
      dplyr::mutate(age_group = cut(age, c((0:15) * 5, Inf), right = FALSE,
                                    labels = diseasystore::age_labels((0:15) * 5))) |>
      dplyr::summarise(population = sum(population), .by = "age_group") |>
      tibble::deframe()


    basis$contacts <- purrr::map2(basis$counts, names(basis$counts), \(lambda, location) {
