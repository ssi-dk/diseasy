...
dir.create("articles/cache/DiseasystoreSeirExample", showWarnings = FALSE)
conn <- RSQLite::SQLite() |>
  DBI::dbConnect(dbname = "articles/cache/DiseasystoreSeirExample/ds.sqlite")
...
map_to_incidence <- list(
  "map" = \(.x, .y) {
    dplyr::mutate(
      .y,
      "incidence" = .x$rate / .x$proportion,
...
map_to_n_infected <- list(
  "map" = \(.x, .y) {
    dplyr::mutate(.y, "n_infected" = .x$rate * .x$population / .x$proportion)
  }
)

map_to_n_positive <- list(
  "map" = \(.x, .y) {
    dplyr::mutate(.y, "n_positive" = 0.65 * .x$rate * .x$population / .x$proportion)
  }
)

map_to_n_admission <- list(
  "map" = \(.x, .y) {
    risk_of_admission <- stats::setNames(c(0.001, 0.01, 0.1), c("00-29", "30-59", "60+"))  # Risk per age group
    delay_distribution <- c(0, 0, 0.2, 0.3, 0.3, 0.1, 0.1) # Must sum = 1

    n_total_admissions <- .x$rate * .x$population / .x$proportion * risk_of_admission[.y$age_group]
...
model_rate_to_observable = list(
  "incidence" = map_to_incidence,
  "n_infected" = map_to_n_infected,
...
model <- DiseasyModelOdeSeir$new(
  ...
  parameters = list(
    ...
    "model_rate_to_observable" = model_rate_to_observable
  )
)
```
