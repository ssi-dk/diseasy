...
cache <- cachem::cache_disk(dir = "article_data/cache/diseasy-immunity")
...
`diseasy` framework may expect named targets such as "infection", "hospitalisation", or "death"[^1].

[^1]: For example, `?DiseasyModelOdeSeir` creates an observable for "hospitalisation" and "death" when they
exist in the given `?DiseasyImmunity` module.
...
- `delay` (optional): The delay of the outcome relative to infection (in days) (used in some models).
- `risks` (optional): The (age-stratified) risk of the outcome following infection (used in some models).
...
![Figure 1: Illustration of the compartmental model structure.](article_data/diseasy-immunity-model-structure.png)
...
[^2]: "time" here is the expectation value of the time of the transition to the compartment with the given
value of $\gamma$
