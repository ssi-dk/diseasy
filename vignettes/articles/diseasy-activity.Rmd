...
The _population contact matrix_ $\overline{\overline{M}}$ with elements $m_{i,j} = t_{i,j} / N_i$ contains the
_mean number of contacts from age group $i$ to age group $j$_ with $N_i$ being the population size of age group $i$.
...
This symmetric variant has the property: $m_{i,j} N_i = m_{j,i} N_j$.[^1] [^2]

[^1]: These are the matrices we get from `socialmixr::contact_matrix()` if we specify `symmetric = TRUE`
[^2]: This is what Klepac (BBC pandemic study) calls the $C$ matrix

The _per capita contact rates_ $\overline{\overline{C}}$ with elements $c_{i,j} = m_{i,j} / N_j = t_{i,j} / (N_i N_j)$.
