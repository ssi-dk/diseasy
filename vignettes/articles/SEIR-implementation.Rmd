...
$$
\dot{\tilde{I}}_i =
\left(\textrm{number of infectious contacts}\right)\left(\textrm{probability of contact being susceptible}\right) -
\left(\textrm{clearing of infection}\right)
$$

To compute the number of infectious contacts we need the number of contacts between infected individual of any age
group and members of the $i$'th age group. If $t_{i,j}$ is the number of contacts from age group $i$ to age group
$j$, then $p \tilde{I}_j t_{i,j} / \tilde{N}_j$ is the number of infectious contacts (infected individual of age group
$j$ times mean number of contacts to age group $i$ with each contact carrying a risk of infection $p$).
...
$$
\tilde{N}c_{i,j} = \frac{\tilde{N}}{N_j} \frac{t_{i,j}}{N_i} = \frac{\tilde{N}}{N_j} m_{i,j}
$$

where $m_{i,j}$ is the output of `$get_scenario_contacts()` and the rescaling $\frac{\tilde{N}}{N_j}$ can be performed
