...
$$
\overline{\psi} = [\underbrace{\begin{matrix} E^1\, \cdots \, E^K \;\; I^1 \, \cdots \, I^L \;\; R^1 \, \cdots \, R^M \end{matrix}}_{\text{Age group 1, Variant 1}}\;\;
...
- $s(t)$: Multiplicative effect of season
- $\epsilon_a$: Relative infectiousness of variant $a$
- $\Gamma$: Overall infection rate scaling factor

$$
\overline{\overline{T}} = \Gamma\;s(t)\;\overline{\overline{\mathcal{E}}} \odot \overline{\overline{CI}}
$$
...
# `infection_matrix`
For the final computation of infections in `rhs`, we construct the matrix `infection_matrix` ($\overline{\overline{f}}$).
...
\end{bmatrix}
$$
Notice that the off-diagonal has zero-elements where the (reduced) state vector changes age groups ($I^k_L$ -> $E_1^{k+1}$).

The form of the transmission matrix is more difficult to write up.

We lump risk modifying factors such as the effect of season $s(t)$ and overall infection rate $\Gamma$ into a single
factor $\beta$.
...
$$
\hat{\rho}_{a} = S + \sum_{b = 1}^V\sum_{m=1}^M \left(1 - \chi_{b \leftarrow a}(1 - \gamma_m)\right) R_m^b
$$
...
$$
\hat{\rho}_{i, a} = S_i + \sum_{b = 1}^V\sum_{m=1}^M \left(1 - \chi_{b \leftarrow a}(1 - \gamma_m)\right) R_m^{i,b}
$$
