...
\sum_i c_{1,i} \sum_l I^l_{i,1} & \cdots & \sum_i c_{1,i} \sum_l I^l_{i,V} \\
\vdots & \ddots & \vdots \\
\sum_i c_{A,i} \sum_l I^l_{i,1} & \cdots & \sum_i c_{A,i} \sum_l I^l_{i,V}
\end{bmatrix}
$$
...
For this purpose, we first expand the `infection_rate` matrix to a $A \cdot (V \cdot (K+L+M) + 1) \times V$ matrix by repeating the
rows of the `infection_rate` matrix.

Each element in the state vector has a corresponding age group. If we write this age group of element $s$ as $i|s$, then
we construct the matrix:
$$
\overline{\overline{U}} = \begin{bmatrix}
  t_{i|s=1, 1} & \cdots & t_{i|s=1, V} \\
  t_{i|s=2, 1} & \cdots & t_{i|s=2, V} \\
  \vdots & \vdots &\vdots
\end{bmatrix}
$$
We now have a matrix, whose elements are the infection rate for each variant matched to the elements of the state vector.
...
$A \cdot (V \cdot (K+L+M) + 1) \times V$ matrix on the form:
...
\overline{\overline{\rho}} = \begin{bmatrix}
  ...
  \begin{bmatrix}
    ...
    1 - \chi_{V\leftarrow V} (1 - \overline{\gamma})
  \end{bmatrix} \\
  \overline{0}_A & \cdots & \overline{0}_A
\end{bmatrix}
$$
Where $\overline{0}_n$ is a zero vector of length $n$.
...
\end{bmatrix}
$$
are $A \cdot (K + L + M) \times 1$ matrices with repeated sub-matrices.
...
\begin{bmatrix}
  \overline{s} & \cdots & \overline{s}
\end{bmatrix} \odot 
\overline{\overline{U}}
$$
Where $\begin{bmatrix}\overline{s} & \cdots & \overline{s}\end{bmatrix}$ is $V$ repeats of $\overline{s}$.

This matrix contains, for each element in the state vector, a row consisting of per-variant infectious contacts
