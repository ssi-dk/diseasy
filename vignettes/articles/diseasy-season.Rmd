...
To check the current status of the module, including which model is set, the `print()` generic can be used:
```{r print}
print(s)
```

Once a season model is set, interfacing with the model happens through the `$model_t()` and `$model_date()` methods.
```{r plot_constant_season_t, fig.alt = "Example plot of $model_t() with the constant_season model."}
t <- 0:365
...
```{r plot_constant_season_date, fig.alt = "Example plot of $model_date() with the constant_season model."}
...
```{r plot_cosine_season_date, fig.alt = "Example plot of $model_t() with the cosine_season model."}
d <- s$reference_date + 0:365
plot(d, purrr::map_dbl(d, s$model_date),
     type = "l", lwd = 2,
     xlab = "",
...
```{r plot_cosine_season_date_w_scale, fig.alt = "Example plot of $model_date() with the cosine_season model."}
s$set_scale(0.25)
