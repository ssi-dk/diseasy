...
$$
\frac{d I^\dag}{d t} = w_1 r_e E_K - w_1 r_i I_1 \\
...
obs <- DiseasyObservables$new(diseasystore = DiseasystoreSeirExample, conn = conn)
...
ggplot2::ggplot(model_data) +
  ...
  ggplot2::scale_color_manual(
    values = c("Infected" = "deepskyblue3", "Test positive (simple)" = "orange", "Test positive (realistic)" = "seagreen")
  ) +
...
incidence_data <- model_data |>
  tidyr::pivot_longer(dplyr::starts_with("n_"), names_to = "source", values_to = "model_output") |>
  dplyr::mutate(
    "incidence" = .data$model_output / (sum(contact_basis$DK$population) * dplyr::if_else(.data$source == "n_infected", 1, 0.65))
  )
```
...
generate_model <- function(K, L, M, rE = 1 / 2.1, rI = 1 / 4.5) {
  ...
  m <- DiseasyModelOdeSeir$new(
    ...
    parameters = list("age_cuts_lower" = c(0, 30, 60), "overall_infection_risk" = 0.025)
  )

  return(m)
}

m <- generate_model(2, 1, 1)
```
...
poly_fit_data <- incidence_data |>
  ...
  dplyr::mutate("t" = as.numeric(.data$date - !!obs$last_queryable_date, units = "days"))
...
incidence_data |>
  ...
  ggplot2::ggplot(ggplot2::aes(x = date, y = incidence)) +
    ...
    ggplot2::geom_vline(
      xintercept = obs$last_queryable_date - m$parameters$incidence_polynomial_training_length,
      linetype = 2, linewidth = 1, color = "red"
    ) +
    ggplot2::ylim(
      0,
      1.1*max(dplyr::pull(dplyr::filter(incidence_data, source == !!source), "incidence"))
    ) +
    ggplot2::facet_wrap(~ age_group) +
    ggplot2::theme_bw()
```
...
```{r}
get_prediction <- function(model, psi, reporting_percentage = 1, signal = "incidence") {
  ...
  if (signal == "incidence") {
    out <- sol_long |>
      ...
      dplyr::mutate(
        ...
        "n_model" = model$disease_progression_rates[["I"]] * model$compartment_structure[["I"]] * .data$value * reporting_percentage * sum(model$activity$contact_basis$population)
      )
  } else if (signal == "prevalence") {
    out <- sol_long |>
      dplyr::filter(stringr::str_starts(.data$state, "I1")) |>
      dplyr::select(!"state") |>
      dplyr::summarise(
  ...
  out <- out |>
    dplyr::mutate(
      "model_configuration" = paste0(names(model$compartment_structure), model$compartment_structure, collapse = "")
    )

  return(out)
}

prediction <- get_prediction(m, psi, reporting_percentage = ifelse(source == "n_infected", 1, 0.65))
...
ggplot2::ggplot() +
  ...
  ggplot2::geom_vline(xintercept = obs$last_queryable_date, linetype = 2, color = "black") +
  ggplot2::facet_grid(source ~ age_group, scales = "free") +
  ggplot2::labs(y = "Model ouput", color = "Model Configuration")
```
...
predictions <- models |>
  purrr::map(\(m) {
    ...
    get_prediction(m, psi, reporting_percentage = ifelse(source == "n_infected", 1, 0.65))
  }) |>
