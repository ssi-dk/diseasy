...
generate_model <- function(K, L, M, rE = 1 / 2.1, rI = 1 / 4.5) {
  ...
  s <- DiseasySeason$new()
  s$set_reference_date(as.Date("2020-01-20"))
  s$use_cosine_season()
...
```{r correctly specified model, fig.alt = "Using a correctly specified model to initialise the SEIR model matches the true data near-perfectly.", eval = rlang::is_installed("duckdb")}
ggplot2::ggplot() +
  ggplot2::geom_point(
    data = incidence_data,
...
```{r configuration test - incidence, include = FALSE, eval = rlang::is_installed("duckdb")}
...
```{r misspecified model increasing, fig.alt = "Using a misspecified model to initialise the SEIR model can match the true data well when infections are increasing.", eval = rlang::is_installed("duckdb")}
models <- list(
...
```{r misspecified model decreasing, fig.alt = "Using a misspecified model to initialise the SEIR model can match the true data well when infections are decreasing.", eval = rlang::is_installed("duckdb")}
