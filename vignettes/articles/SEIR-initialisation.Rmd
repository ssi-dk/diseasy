...
cache <- cachem::cache_disk(dir = "article_data/cache/SEIR-initialisation")
...
```{r SEIR-structure, echo = FALSE, fig.align = "center", fig.cap = "SEIR model overview with multiple E and I states", out.width = "120%"}
knitr::include_graphics(
  devtools::package_file("vignettes/articles/article_data/SEIR-initialisation.png")
)
```

Let us further assume that we have a incidence signal[^1], $Inc^*(t)$, which we would like our model to match.
...
$$
$$
\frac{d Inc^*}{d t} = r_i r_e E_K - r_i Inc^*
$$

If we take the second derivative, we find:
$$
\frac{d^2Inc^*}{d t^2} = r_i r_e \frac{d E_K}{d t} - r_i \frac{d I_l}{d t}
$$
From here, we can inject $\frac{d E_K}{d t}$ from the SEIR equations which in turn relates to $E_{k-1}$.
$$
\frac{d^2Inc^*}{d t^2} = r_i r_e \left(r_e E_{K-1} - r_e E_K\right) - r_i \frac{d I_l}{d t}
$$

This process can be iterated through the derivatives until all $E_K$ states are expressed in terms of $Inc^*(t)$ and
its derivatives.
...
To see why, start the equation for the derivative of $Inc^*(t)$:
$$
\frac{d Inc^*}{d t} = r_i r_e E_K - r_i Inc^*
$$

And separating the $E_k$ and $Inc^*$ terms:
$$
r_i r_e E_K = r_i Inc^* + \frac{d Inc^*}{d t}
$$
...
$$
\frac{d^2Inc^*}{d t^2} = r_i r_e \frac{d E_K}{d t}- r_i \frac{d Inc^*}{d t} \Rightarrow
$$
$$
...
And separating the $E_k$ and $Inc^*$ terms:
$$
r_i r_e^2 E_{k-1} = r_e\left(r_i \frac{d Inc^*}{d t} + Inc^*\right) + r_i \frac{d Inc^*}{d t} + \frac{d^2Inc^*}{d t^2}
$$
...
Since we assume that the signal $Inc^*$ only relates to $I_1$, then we can determine the $I_{l>1}$ states by
evaluating the signal at $Inc^*(t - (l - 1) / r_i)$.
...
We again assume $Inc^* = r_i I_1$.

We can modify the SEIR equation to take this signal as a forcing function with one less $I$ state
(no $I_1$ state).

The equations are as normal except for the following changes:
$$
I = \frac{Inc^*}{r_i} + \sum_{l=2}^L I_l
$$

$$
\frac{d I_2}{d t} = Inc^* - r_i I_2
$$

If we create such a submodel and start this system at a time where there are no new infections,
we can initialize $I_l = 0$, and run the simulation forward to estimate the $S$ and $R$ populations
at the point of interest.

# Initialising custom model output states
When the model is asked to deliver a prediction for an observable, these observables
may be shifted in time (i.e. delayed) relative to the point of infection.

For example, hospitalisation might occur on average 7 days after infection.
If the models starts at `time = 0`, then any model output for hospitalisation
will therefore have a transient period of 7 days before that signal begins being
exported by the model.

Each model output is derived either from the $I_1$ state or any of the $C$ states
added when the user defines a custom model output (see the "DiseasyModelOde" article).

To eliminate the transient behaviour of the model output when delays are included,
we can determine the values of the $I_1$ state and the $C$ states for `time < 0`.

This process is two-fold,
1) To determine $I_1$ for `time < 0` we can simply use the input signal given to
the initialisation method by re-using the assumption $Inc^* = r_i I_1$.

2) To determine $C_\cdot$ for `time < 0` we use the same submodel used to infer the R
and S states and configure the custom model outputs here.
The forcing function in the submodel thus generates the time evolution of the
$C_\cdot$ states for `time < 0`.
...
```{r, eval = rlang::is_installed("duckdb")}
...
```{r, example data, fig.alt = "Plots of the example data bundled with diseasy.", eval = rlang::is_installed("duckdb")}
...
incidence_data <- obs$get_observation(
  ...
  stratification = rlang::quos(age_group, region = "All")
) |>
...
```{r, incidence fitting, fig.alt = "Fitting a polynomial to the incidence data to estimate derivatives.", eval = rlang::is_installed("duckdb")}
...
```{r, eval = rlang::is_installed(c("duckdb", "optimx", "ucminf"))}
...
```{r correctly specified model, fig.alt = "Using a correctly specified model to initialise the SEIR model matches the true data near-perfectly.", eval = rlang::is_installed(c("duckdb", "optimx", "ucminf"))}
ggplot2::ggplot() +
  ggplot2::geom_point(
    data = incidence_data,
...
```{r configuration test - incidence, include = FALSE, eval = rlang::is_installed(c("duckdb", "optimx", "ucminf"))}
...
observable <- "incidence"
x <- m$get_results(observable = observable, prediction_length = 60)
y <- m$get_data(
...
```{r misspecified model increasing, fig.alt = "Using a misspecified model to initialise the SEIR model can match the true data well when infections are increasing.", eval = rlang::is_installed(c("duckdb", "optimx", "ucminf"))}
models <- list(
...
```{r misspecified model decreasing, fig.alt = "Using a misspecified model to initialise the SEIR model can match the true data well when infections are decreasing.", eval = rlang::is_installed(c("duckdb", "optimx", "ucminf"))}
...
$Inc^*(t)$. Specifically, $Inc^*(t)$ should be the number of true infections in the group divided by the total population not the grouped population.
