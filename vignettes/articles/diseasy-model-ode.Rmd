...
# We will destroy the cache if the source code for `DiseasyImmunity`,
# `DiseasyObservables` or `DiseasyModelOdeSeir` changes
cache <- cachem::cache_disk(dir = "article_data/cache/diseasy-model-ode")
...
source_code_hash <- list(
  purrr::map(
    c(DiseasyBaseModule, DiseasyImmunity, DiseasyObservables, DiseasyModelOdeSeir, DiseasystoreSeirExample),
    r6_hash
  ),
  seir_example_data
) |>
...
last_queryable_date <- observables$ds$min_start_date + 45
```
...
# To stratify the population, we need to define the population
...
This particular scenario also includes simple seasonality relationship:
```{r diseasy_season}
# Season scenario
...
## Immunity scenario
This scenario includes an exponentially waning immunity against infection:
```{r diseasy_immunity}
...
In this example, we define maps from the incidence to the number of positive cases.
This is a counting observables and the default *reduce* function can be used.
...
### Custom model output
The `diseasy` ODE model templates uses a generic system for generating outputs
from the state_vector, $\overline{\psi}$.

A output can be configured to either derive from the full state vector
$\overline{\psi}$ or the infection matrix $f$ (see the article `SEIR-implementation`
in the online documentation).

For example, if the user wants to configure an output for the prevalence of the
disease rather than the incidence, a output summing across the $I$ states in the
model can be used.

Alternatively, if the user wants to configure outputs related to infections,
such as hospitalisation, a outputs summing across the infection matrix with
age-specific risks of hospitalisation can be used.

In either case, the output is configured by defining a vector of weights.
...
element the output is set to track.

In the former case, the surveillance states are derived from the state_vector whereas
in the latter the output is derived from the new infections occurring in the model.

In the case of the "n_infected" output defined by default in the model, the weights
are essentially a matrix with weights only for each $I_1$ state[^2] and the output
tracks the state_vector.

[^2]: The "n_infected" output does not actually create the additional surveillance states in the implementation
...
As stated above, the model always provides a prediction for the "n_infected" output
but there are cases where other outputs are automatically configured by `diseasy`.

For example, when the model includes a `?DiseasyImmunity` module, outputs are
configured to track outcomes related to immunity.

Here, waning targets (other than "infection") in the immunity scenario are
assigned their own output which are derived from the infection matrix
...
We will configure such an output by adding a non-waning target called "admission" to `DiseasyImmunity`.
```{r diseasy_immunity_output}
immunity$set_no_waning(
  target = "admission",
  delay = 3.6, # Average delay in the scenario
  risks = c("00-29" = 0.001, "30-59" = 0.01, "60+" = 0.1)
)
```
...
observable <- "incidence"
x <- model$get_results(observable = observable, prediction_length = 60)
y <- model$get_data(
  observable = observable, prediction_length = 60, period = "plotting"
) |>
  dplyr::filter(.data$t > 0)
stopifnot(
  "Model prediction varies more than expected from example data" = {
    purrr::every(
      abs(cumsum(x[[observable]]) - cumsum(y[[observable]])) /
        cumsum(x[[observable]]),
      ~ . < 0.6
    )
  }
