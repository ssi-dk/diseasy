...
stopifnot(
  "Model prediction varies more than expected from example data" = {
    purrr::every(abs(x$incidence - y$incidence) / y$incidence, ~ . < 0.3)
  }
)
```
...
stopifnot(
  "Model prediction varies more than expected from example data" = {
    purrr::every(abs(x$n_infected - y$n_infected) / y$n_infected, ~ . < 0.3)
  }
)
```
...
stopifnot(
  "Model prediction varies more than expected from example data" = {
    purrr::every(abs(x$n_positive - y$n_positive) / y$n_positive, ~ . < 0.3)
  }
)
```
...
stopifnot(
  "Model prediction varies more than expected from example data" = {
    purrr::every(abs(x$n_admission - y$n_admission) / y$n_admission, ~ . < 0.3)
  }
)
```
