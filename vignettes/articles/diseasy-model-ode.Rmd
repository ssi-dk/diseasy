...
3) **Mapping to observables.**
  The ODE family of models is in some ways bespoke in the sense that the user needs to provide a mapping from and to
  real-world observables.

  We distinguish between two mappings:
  First, observed real-world data, such as positive tests or hospitalisations, is used to estimate the "true" incidence
  of infections.
  Secondly, the modelled incidence is then mapped back to the observable quantities of interest.

  The model _must_ have an estimate of the disease incidence to be initialised.
  If no reverse mappings are given, the model can only provide predictions for the true incidence and true number of
  infected.
...
The first step to creating a `?DiseasyModelOdeSeir` model is to define the model scenario.
In this case, as we stated above, we can exactly match the model scenario to the data since
the data was created using the same class of models.


First we define the population to include in the model via the `?DiseasyPopulation`
module, and then we define the activity scenario via the `?DiseasyActivity` module,
before finally defining the seasonality scenario via the `?DiseasySeason` module.

## Model population
In this example scenario, we use a simple three age-group stratification of the population in the model.

```{r diseasy_population}
# Split the model population into three age groups
...
This particular scenario also includes simple seasonality and waning immunity:
```{r diseasy_season}
...
- *disease_progression_rates*: A named vector specifying the progression rates
  from $E$ to $I$ and from $I$ to $R$ (i.e. waiting times). The rates determine the expected time spent in each state).
- *overall_infection_risk*: A population-level scaling factor for transmission, controlling the overall risk of infection in the population.

```{r model configuration, eval = rlang::is_installed(c("RSQLite", "optimx", "ucminf"))}
# Hyper parameters
...
infections in the population. These dynamics are not usually observed directly. Instead, they must be inferred from one or more observed data sources, such as positive tests or hospital admissions.

Mathematically, we map from the set of real-world observables $\vec{O}(t)$ to the
incidence $Inc(t)$ via the map $f$:
$$\vec{O}(t) \overset{f}{\rightarrow} Inc(t)$$
The model then produces a number of observations under the given configuration.

By default, the model will predict the evolution of the incidence[^1], $\tilde{Inc}(t)$,
...
From the model incidence $\tilde{Inc}(t)$, we then map to the observable estimates
$\tilde{O}_i(t)$ via the maps $g_i$:
$$\tilde{Inc}(t) \overset{g_1}{\rightarrow} \tilde{O}_1(t)$$
$$\tilde{Inc}(t) \overset{g_2}{\rightarrow} \tilde{O}_2(t)$$
$$\vdots$$
$$\tilde{Inc}(t) \overset{g_n}{\rightarrow} \tilde{O}_n(t)$$
...
attr(map_to_n_positive$map, "delay") <- 14
  ...
  "map" = \(.x, .y) {
    ...
    data.frame(
      "date" = as.vector(
        outer(
          X = .x$date,
          Y = seq_along(delay_distribution) - 1,
          FUN = "+"
        )
      ),
      "n_admission" = as.vector(
        outer(
          X = .x$n_infected * risk_of_admission[.y$age_group],
          Y = delay_distribution,
          FUN = "*"
        )
      )
    )
...
attr(map_to_n_admission$map, "delay") <- 7 # Length of `delay_distribution`
...
A observable can be configured to either derive from the full state vector
$\overline{\psi}$ or the infection matrix $f$ (see the article `SEIR-implementation`
in the online documentation).

For example, if the user wants to configure an observable for the prevalence of the
disease rather than the incidence, a observable summing across the $I$ states in the
model can be used.

Alternatively, if the user wants to configure observables related to infections,
such as hospitalisation, a observables summing across the infection matrix with
age-specific risks of hospitalisation can be used.
...
In the former case, the surveillance states are derived from the state_vector whereas
...
assigned their own observables which are derived from the infection matrix
...
```{r model creation, eval = rlang::is_installed(c("RSQLite", "optimx", "ucminf"))}
...
```{r, eval = rlang::is_installed(c("RSQLite", "optimx", "ucminf"))}
model$get_results(
...
```{r, fig.alt = "Model prediction for the number of incidence - stratified by age group.", eval = rlang::is_installed(c("RSQLite", "optimx", "ucminf"))}
plot(
  model,
  observable = "incidence",
...
```{r configuration test - incidence, include = FALSE, eval = rlang::is_installed(c("RSQLite", "optimx", "ucminf"))}
...
stopifnot(
  "Model prediction varies more than expected from example data" = {
    purrr::every(abs(x$incidence - y$incidence) / x$incidence, ~ . < 0.5)
  }
)
```
...
```{r, fig.alt = "Model prediction for the number of infected - stratified by age group.", eval = rlang::is_installed(c("RSQLite", "optimx", "ucminf"))}
plot(
  model,
  observable = "n_infected",
  prediction_length = 60
...
```{r configuration test - n_infected, include = FALSE, eval = rlang::is_installed(c("RSQLite", "optimx", "ucminf"))}
# This code block is used as a sanity check that the models work as expected
# When the model works, we should be able to closely match model with data
# - when not, we get informed by an error
...
```{r, fig.alt = "Model prediction for the number of positive.", eval = rlang::is_installed(c("RSQLite", "optimx", "ucminf"))}
plot(
  model,
  observable = "n_positive",
  prediction_length = 60
...
```{r configuration test - n_positive, include = FALSE, eval = rlang::is_installed(c("RSQLite", "optimx", "ucminf"))}
# This code block is used as a sanity check that the models work as expected
# When the model works, we should be able to closely match model with data
# - when not, we get informed by an error
...
stopifnot(
  "Model prediction varies more than expected from example data" = {
    purrr::every(abs(x$n_positive - y$n_positive) / x$n_positive, ~ . < 0.5)
  }
)
```
...
```{r, fig.alt = "Model prediction for the number of positive - stratified by age group.", eval = rlang::is_installed(c("RSQLite", "optimx", "ucminf"))}
plot(
  model,
  observable = "n_positive",
  prediction_length = 60,
...
```{r, fig.alt = "Model prediction for the number of hospital admissions.", eval = rlang::is_installed(c("RSQLite", "optimx", "ucminf"))}
plot(
  model,
  observable = "n_admission",
  prediction_length = 60
...
```{r configuration test - n_admission, include = FALSE, eval = rlang::is_installed(c("RSQLite", "optimx", "ucminf"))}
# This code block is used as a sanity check that the models work as expected
# When the model works, we should be able to closely match model with data
# - when not, we get informed by an error
...
```{r, fig.alt = "Model prediction for the number of hospital admissions - stratified by age group.", eval = rlang::is_installed(c("RSQLite", "optimx", "ucminf"))}
plot(
  model,
  observable = "n_admission",
  prediction_length = 60,
