...
# When the model works, we should be able to closely match model with data - when not, we get informed by an error
x <- model$get_results(observable = "incidence", prediction_length = 60)
y <- dplyr::filter(model$get_data(observable = "incidence", prediction_length = 60, period = "plotting"), .data$t > 0)
stopifnot(purrr::every(abs(x$incidence - y$incidence) / y$incidence, ~ . < 0.3))
```
...
y <- dplyr::filter(model$get_data(observable = "n_infected", prediction_length = 60, period = "plotting"), .data$t > 0)
stopifnot(purrr::every(abs(x$n_infected - y$n_infected) / y$n_infected, ~ . < 0.3))
```
...
y <- dplyr::filter(model$get_data(observable = "n_positive", prediction_length = 60, period = "plotting"), .data$t > 0)
stopifnot(purrr::every(abs(x$n_positive - y$n_positive) / y$n_positive, ~ . < 0.3))
```
...
y <- dplyr::filter(model$get_data(observable = "n_admission", prediction_length = 60, period = "plotting"), .data$t > 0)
stopifnot(purrr::every(abs(x$n_admission - y$n_admission) / y$n_admission, ~ . < 0.3))
```
