...
# Motivation
From our testing, we have learned that our method used to express the waning immunity in a compartmental model[^1]
...
the algorithm is "eliminated" and no more computation is done for that algorithm. Furthermore, if the value (error) of
the objective function for the optimiser is equal or greater than `1e3` it is also "eliminated".
Note that this is done on per-method basis as we know the optimisation algorithms fare differently for the different
methods for approximation.

Once the round is complete, we update the list of eliminated algorithms and then we run the optimisation of $M + 1$ with
the reduced set of algorithms.

The entire optimisation process is run for three configurations of `monotonous` and `individual_level`:
- `monotonous = 0` and `individual_level = 0`
- `monotonous = 1` and `individual_level = 0`
- `monotonous = 1` and `individual_level = 1`
see `vignette("diseasy-immunity")` for `monotonous` and `individual_level` definitions.
...
results <- diseasy_immunity_optimiser_results |>
  dplyr::mutate(
    "monotonous" = dplyr::if_else(.data$monotonous, "yes", "no"),
    "individual_level" = dplyr::if_else(.data$individual_level, "yes", "no")
  )
...
best_optim_results <- results |>
  ...
  dplyr::summarise(
    ...
    .by = c(
      ...
      "optim_method", "strategy"
    )
  ) |>
  dplyr::filter(
    .data$n_targets == max(.data$n_targets),
    .by = c("target", "method", "monotonous", "individual_level")
  )
...
below.
Here we present the top 5 "best" optimiser/strategy combination for each method and `monotonous`/`individual_level` setting.
...
```{r helper functions for gt, include = FALSE, eval = rlang::is_installed("gt")}
...
bold_min_value <- function(gt_table) {
  min_value <- gt_table$`_data` |>
    ...
    tidyr::pivot_longer(
      !c("row_id", "monotonous", "individual_level")
    ) |>
    dplyr::slice_min(
      .data$value,
      with_ties = FALSE,
      by = c("monotonous", "individual_level")
    ) |>
    dplyr::mutate("name" = stringr::str_remove(.data$name, "^value/"))

  purrr::reduce(
    seq_len(nrow(min_value)),
    \(gt_table, row_index) {
      gt_table |>
        gt::tab_style(
          style = list(gt::cell_text(weight = "bold")),
          locations = gt::cells_body(
            columns = dplyr::ends_with(min_value$name[[row_index]]),
            rows = min_value$row_id[[row_index]]
          )
        )
    },
    .init = gt_table
...
```{r, echo = FALSE, results = "asis", eval = rlang::is_installed("gt")}
total_results <- best_optim_results |>
  ...
  dplyr::summarise(
    ...
    .by = c("monotonous", "individual_level", "method", "strategy", "optim_method")
  ) |>
  dplyr::mutate(
  ...
  dplyr::slice_max(
    .data$n_targets,
    by = c("monotonous", "individual_level", "method")
  )
...
total_results |>
  ...
  tidyr::unnest_longer(!c("monotonous", "individual_level")) |>
...
chosen_defaults <- tibble::tibble(
  "method" = character(0),
  "monotonous" = character(0),
  "individual_level" = character(0),
...
```{r, echo = FALSE, eval = rlang::is_installed("gt")}
total_results_for_plotting <- total_results |>
  dplyr::summarise(
...
visualise_methods <- function(method, total_results = total_results_for_plotting) {

  # Form a subset to plot
  gg_data <- total_results |>
    ...
    dplyr::left_join(
      ...
      by = c("method", "monotonous", "individual_level", "strategy", "optim_method")
    ) |>
    dplyr::mutate(
  ...
  ggplot2::ggplot() +
    ...
    ggplot2::facet_wrap(
      ggplot2::vars(.data$monotonous, .data$individual_level),
...
```{r overview free_delta, echo = FALSE, fig.alt = "Total error vs execution time for the free_delta method.", eval = rlang::is_installed("gt")}
visualise_methods("free_delta")
```

```{r overview free_gamma, echo = FALSE, fig.alt = "Total error vs execution time for the free_gamma method.", eval = rlang::is_installed("gt")}
visualise_methods("free_gamma")
```

```{r overview all_free, echo = FALSE, fig.alt = "Total error vs execution time for the all_free method.", eval = rlang::is_installed("gt")}
visualise_methods("all_free")
```


In total, we find that there is no clear choice for best, general optimiser/strategy combination.

For the defaults, we have chosen the optimiser/strategy combination dependent on the method of parametrisation and the
values of `monotonous` and `individual_level`.

The following optimiser/strategy combinations act as the currently selected defaults for the `?DiseasyImmunity` class.
No default is assigned yet for `monotonous = yes`, `individual_level = no`; the new results are included in the
comparisons above so that this choice can be made from the benchmark.
```{r, echo = FALSE, results = "asis", eval = rlang::is_installed("gt")}
chosen_defaults |>
  gt::gt() |>
  gt::tab_style(
...
split_data <- per_target_results |>
  ...
  dplyr::select(
    "target", "monotonous", "individual_level",
    "method", "strategy", "value", "optim_method"
  ) |>
  dplyr::arrange(
    .data$target,
    .data$monotonous,
...
gt_tables <- split_data |>
  purrr::imap(~
    .x |>
      ...
      tidyr::unnest_longer(!c("target", "monotonous", "individual_level")) |>
      dplyr::select(!dplyr::ends_with("_id")) |>
      gt::gt() |>
      gt::tab_spanner(
        label = "free_delta",
