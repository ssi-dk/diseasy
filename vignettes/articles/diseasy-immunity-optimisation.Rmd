...
chosen_defaults <- tibble::tibble(
  "method" = character(0), "penalty" = character(0), "strategy" = character(0), "optim_method" = character(0)
) |>
  tibble::add_row("method" = "free_delta", "penalty" = "no",  "strategy" = "naive",       "optim_method" = "ucminf") |>
  tibble::add_row("method" = "free_delta", "penalty" = "yes", "strategy" = "naive",       "optim_method" = "tnewton") |>
  tibble::add_row("method" = "free_gamma", "penalty" = "no",  "strategy" = "naive",       "optim_method" = "subplex") |>
  tibble::add_row("method" = "free_gamma", "penalty" = "yes", "strategy" = "naive",       "optim_method" = "hjkb") |>
  tibble::add_row("method" = "all_free",   "penalty" = "no",  "strategy" = "combination", "optim_method" = "neldermead") |>
  tibble::add_row("method" = "all_free",   "penalty" = "yes", "strategy" = "naive",       "optim_method" = "ucminf")
```
...
visualise_methods <- function(total_results, method) {
  gg_data <- total_results |>
    dplyr::slice_min(.data$value, n = 6, with_ties = FALSE, by = c("penalty", "method")) |>
    dplyr::left_join(
      dplyr::mutate(chosen_defaults, "default" = TRUE), 
      by = c("method", "penalty", "strategy", "optim_method")
    ) |>
    dplyr::mutate("default" = dplyr::if_else(!is.na(.data$default), "Yes", "No")) |>
    dplyr::filter(.data$method == !!method) |>
    tidyr::unite("optim_strategy", "strategy", "optim_method", sep = "-")
  
  palette <- purrr::reduce(purrr::map(c("Dark2", "Set1", "Set2"), ~ RColorBrewer::brewer.pal(8, .)), c) |>
    stats::setNames(unique(gg_data$optim_strategy))
  palette <- purrr::discard_at(palette, is.na(names(palette))) # discard_at with is.na directly fails for whatever reasons
  
  ggplot2::ggplot() +
    ggplot2::geom_point(
      data = gg_data,
      mapping = ggplot2::aes(x = execution_time, y = value, color = optim_strategy, fill = optim_strategy, shape = default),
      size = 3
    ) + 
    ggplot2::geom_hline(
    ...
    ggplot2::facet_wrap(~ penalty, labeller = ggplot2::labeller(.cols = ggplot2::label_both), scales = "free") + 
    ggplot2::labs(
      x = "Execution time (hours)", 
      y = "Total error", 
      title = glue::glue("Best optimisers for {method}"),
      color = "strategy-optim_method",
      fill = "strategy-optim_method",
    ...
    ggplot2::scale_shape_manual(values = c("Yes" = 23, "No" = 21)) +
    ggplot2::scale_x_log10()
}
```

```{r overview free_delta, include = FALSE, fig.alt = "Total error vs execution time for the free_delta method."}
visualise_methods(total_results, "free_delta")
```
```{r overview free_gamma, include = FALSE, fig.alt = "Total error vs execution time for the free_gamma method."}
visualise_methods(total_results, "free_gamma")
```

```{r overview all_free, include = FALSE, fig.alt = "Total error vs execution time for the all_free method."}
