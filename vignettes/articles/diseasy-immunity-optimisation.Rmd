...
results <- diseasy_immunity_optimiser_results |>
  dplyr::mutate(
    "monotonous" = dplyr::if_else(.data$penalty >= 0.5, "yes", "no"),
    "individual_level" = dplyr::if_else(.data$penalty > 0.5, "yes", "no")
  ) |>
...
best_optim_results <- results |>
  ...
  dplyr::summarise(
    ...
    "n_targets" = dplyr::n(),
    .by = c("target", "method", "monotonous", "individual_level", "optim_method", "strategy")
  ) |>
  dplyr::filter(
...
total_results <- best_optim_results |>
  ...
  dplyr::slice_max(.data$n_targets, by = c("monotonous", "individual_level", "method"))
...
visualise_methods <- function(total_results, method) {
  ...
  ggplot2::ggplot() +
    ...
    ggplot2::facet_wrap(
      ggplot2::vars(monotonous, individual_level),
...
split_data <- per_target_results |>
  ...
  dplyr::select(
    "target", "monotonous", "individual_level", "method", "strategy", "value", "optim_method"
  ) |>
  dplyr::arrange(
    .data$target,
    .data$monotonous,
