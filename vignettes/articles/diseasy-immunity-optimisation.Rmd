...
add_footnote <- function(gt_table) {
  ...
  if ("auglag_slsqp" %in% optimisers) {
    caption <- c(caption, "auglag_slsqp: auglag with SLSQP as local solver.")
  }
  if ("auglag_mma" %in% optimisers) {
    caption <- c(caption, "auglag_mma: auglag with MMA as local solver.")
  }
...
total_results <- marginal_results |>
  dplyr::summarise(
    "value" = sum(.data$value),
    "execution_time" = sum(.data$execution_time) / 60,
...
```{r gg_data, echo = FALSE}
gg_data <- function(total_results, method, n = 5) {
  total_results |>
    dplyr::slice_min(
      (.data$value - min(.data$value)) / median(.data$value) +
        (.data$execution_time - min(.data$execution_time)) / median(.data$execution_time),
      n = n,
      with_ties = FALSE,
      by = c("penalty", "method")
    ) |>
...
optimisers <- purrr::map(c("free_delta", "free_gamma", "all_free"), ~ gg_data(total_results, .)) |>
  purrr::list_rbind() |>
  dplyr::pull("optim_strategy") |>
  unique()
...
palette <- purrr::reduce(
  ...
  purrr::discard(~ . == "#FFFF33") |>
  stats::setNames(optimisers)

palette <- palette[!is.na(names(palette))]
palette <- c(palette, "other" = "grey40")
```

```{r visualise_methods, echo = FALSE}
visualise_methods <- function(total_results, method) {
  best <- gg_data(total_results, method)
  all <- gg_data(total_results, method, n = Inf)
  focus <- all |>
    dplyr::filter(
      .data$execution_time <= max(best$execution_time),
      .data$value <= max(best$value)
    ) |>
    dplyr::mutate(
      "optim_strategy" = factor(
        dplyr::if_else(
          .data$optim_strategy %in% best$optim_strategy,
          .data$optim_strategy,
          "other"
        ),
  ...
  ggplot2::ggplot(
    ...
    ggplot2::geom_hline(
      data = dplyr::summarise(all, "value" = min(.data$value), .by = "penalty"),
      mapping = ggplot2::aes(yintercept = .data$value, linetype = "Smallest observed error"),
      color = "black"
    ) +
    ggplot2::geom_point(data = focus, size = 4) + 
    ggplot2::facet_wrap(
      ~ penalty,
    ...
    ggplot2::labs( 
      ...
      y = "Total error (excluding penalties)",
      title = glue::glue("Optimisation trade-off for method `{method}`"),
      color = "Configuration",
...
per_target_results <- marginal_results |>
  dplyr::mutate(
    "value" = round(.data$value, 2)
  )
