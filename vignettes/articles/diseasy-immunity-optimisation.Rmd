...
| Sum of exponentials | $0.3 \cdot 0.8 \cdot \frac{\exp(-0.5 t / \tau) + \exp(-2 t / \tau) + \exp(-3 t / \tau)}{2}$                |
...
the algorithm is "eliminated" and no more computation is done for the algorithm. Accordingly if the value (error) of the
objective function for the optimiser is equal or greater than `1e3` it is "eliminated". Note that this is done on per
method basis as we know the optimisation algorithms fare differently on the different methods for approximation.
