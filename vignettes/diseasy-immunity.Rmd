...
- `individual_level`: A penalty variation in $\gamma_m$ and $\delta_m$ values are added.
   ...
   smoothen transitions between states.
...
```{r effect of penalty, fig.alt = "Adding penalty to approximations smoothens the transitions between compartments."}  
im$set_custom_waning(
...
approximation <- im$approximate_compartmental(method = "all_free", M = 5)
plot(
  x = cumsum(c(0, 1 / approximation$delta)), 
  y = approximation$gamma$infection, 
  col = "red", 
  xlab = "Time", ylab = "gamma",
  ylim = c(0, 1), pch = "16"
)

approximation <- im$approximate_compartmental(
  method = "all_free", 
  M = 5, 
  monotonous = FALSE, 
...
points(cumsum(c(0, 1 / approximation$delta)), approximation$gamma$infection, pch = "16")
```
