...
- Approach 2: (`gamma_fixed_step`) - $\gamma_n$ values are monotonously decreasing ($\gamma_n = (n-1)/(N-1)$),
...
# Prepare the data
N_seq <- seq(from = 2, to = 7, by = 1)                                                                                  # nolint: object_name_linter
...
results <- tidyr::expand_grid(
  ...
  purrr::pmap(
    \(method, N) {                                                                                                      # nolint: object_name_linter
      im$approximate_compartmental(method = method, N = N, optim_control = data.frame("method" = "nlm", "maxit" = 100))
    },
