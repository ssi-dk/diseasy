...
im$approximate_compartmental(method = "free_gamma", M = 2)
```
...
if (!interactive() && cache$size() > 8) {
  stop(glue::glue("Extra cache items created (cache size: {cache$size()}) -- hashing issue?"))
}
