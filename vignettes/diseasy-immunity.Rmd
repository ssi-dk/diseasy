...
source_code_hash <- digest::digest(DiseasyImmunity)
...
withr::local_options(
  ...
  "diseasy.cache" = cache # Cache for 24 hours
)
```
...
- In `DiseasyImmunity` we operate with multiple targets, so we have a set of `\gamma_n` values for each target, but
  only a single set of transition rates `\delta_n`.
  
Combined, these rates and risks define a population level waning immunity. From the waiting time distributions, hypoexponential in general, Erlang distributed if all rates are equal, we can compute the probability to occupy each
compartment at time $t$ after entering the first compartment at $t = 0$. We denote this probability as $\textrm{P}_n(t \;|\;\vec{\delta})$. Given this occupancy probability, we can parametrise the approximated waning immunity of the $k$th target as:

$$
\tilde{f}^k(t) \equiv \sum_{n = 1}^N \gamma_n^k\; \textrm{P}_n(t\;|\;\vec{\delta})
$$
...
- Strategy 1 (`naive`): The initial guess that `\gamma_n` values are linearly distributed from $f(0)$ to $f(\infty)$,
  while the `\delta_n` values are assumed to be set by the reciprocal of the median time scale of the targets.
  
- Strategy 2 (`recursive`): The initial guess is based on the previous optimisation results for `N - 1`.
...
$$
 \textrm{obj} = \sum_{k = 1}^K \sqrt{\int_0^\infty \left( f^k(t) - \tilde{f}^k(t) \right)^2} + \textrm{penalty}
$$

where the penalties are controllable via the `monotonous` and `individual_level` arguments:

- `monotonous`: A penalty for non-monotonous $\gamma_n$ values is added.

- `individual_level`: A penalty variation in $\gamma_n$ and $\delta_n$ values are added.
...
ggplot2::ggplot(results, ggplot2::aes(x = N, y = value, color = method)) +
  ...
  ggplot2::coord_cartesian(xlim = c(1, max(results$N)), expand = FALSE)
```
