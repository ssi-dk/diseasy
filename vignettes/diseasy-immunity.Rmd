...
# Clean old instance and create a new
rm(im)
devtools::load_all()
im <- DiseasyImmunity$new()

# Prepare the data
N_seq <- seq(from = 1, to = 2, by = 1)
...
results <- map(approaches, function(approach) {
  map(N_seq, function(N) {
    res <- im$approximate_compartmental(approach = approach, N = N)$value
    setNames(list(res[[1]]), names(res))
  })
}) %>% flatten() %>% reduce(c) #reduces nested list by one level and combines lists ino a single flat list
