...
function based on the specified `model_name` and `target`.

If `model_name` is a `function`, it will be set as a custom waning function via `$set_custom_waning()`.
If instead it is `character`, the corresponding to `$set_<model_name>()` method will be used to set the waning model.
...
`DiseasyImmunity` can also provide the approximations of the waning models for com compartmental ODE models.
