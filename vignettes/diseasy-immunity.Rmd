...
source_code_hash <- c("public_methods", "public_fields", "private_methods", "private_fields", "active") |>
  purrr::map( ~ purrr::pluck(DiseasyImmunity, .x)) |>
  purrr::reduce(c) |>
  purrr::map_if(checkmate::test_function, ~ deparse(rlang::fn_body(.))) |>
  digest::digest()
...
$\textrm{P}_n(t \;|\;\vec{\delta})$. Given this occupancy probability, we can parametrise the approximated waning
...
im$approximate_compartmental(method = "free_gamma", N = 4)
```
