...
```{r null_scenario}
act$get_scenario_freedom()
```

To use the module, we must define the activity scenario. In the example below, we use configure the module to use
...
The `DiseasyActivity` module is designed around using counts of contacts for these matrices.
This way, the matrices can be dynamically expressed in various age groups according to the needs of the models.

To retrieve contact matrices from the module, we use the `$get_scenario_contacts()` method:
```{r scenario_contacts}
...
contact_counts <- act$get_scenario_contacts(age_cuts_lower = c(0, 20, 40, 60, 80), weights = c(1, 1, 1, 1))
print(contact_counts)
```

If we want to convert the contact matrices to rates rather than counts, we can use the `$rescale_counts_to_rates()`
method.

By default, the methods uses the population in the loaded `contact_basis` to compute the population in the age groups of the contact matrix. Custom populations can be supplied through the `pop` argument.

```{r scenario_contacts_rates}
act$rescale_counts_to_rates(contact_counts)
```
...
The degree of freedom can be queried similarly to the contact matrices:
```{r scenario_freedom_weighted}
act$get_scenario_freedom(age_cuts_lower = c(0, 20, 40, 60, 80), weights = c(1, 1, 1, 1))
```
