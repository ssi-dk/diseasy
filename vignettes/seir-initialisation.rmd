...
# Adding some noise
pop = 1e3
II <- rI * purrr::map_dbl(I, \(lambda) rpois(1, pop * lambda)) / pop

polyfit <- \(offset) lm(
  I ~ poly(t, 3, raw = TRUE),
  data = data.frame(t = head(t, 21) - t0 + offset, I = head(II, 21))
)
...
```{r}
check_solution <- function(EI_states, K, L, rE, rI, offset = 0, signal = "test") {
  # Run a SIR model to generate the target of our initialisation
  m <- DiseasyModelOdeSeir$new(
    season = T, activity = T,
    observables = DiseasyObservables$new(
  ...
  times <- seq(0, 10, length.out = 100) + offset - 7
  
  test_sol <- deSolve::ode(
    y = c(EI_states / sum(EI_states) * (1- R[21] - S[21]), R[21], S[21]),
    times = times, 
    func = private %.% rhs, 
    parms = NULL, 
  ...
  if (signal == "test") {
    plot(t, rI * I, xlim = c(0, t0 + max(times)))
    lines(t0 + times, L * rI * test_sol[, K + 2], col = "red")    
  } else if (signal == "prevalence") {
    plot(t, I, xlim = c(0, t0 + max(times)))
    lines(t0 + times, rowSums(test_sol[, (K + 2):(K + L + 1), drop = FALSE]), col = "red")
  }
}
```
...
```{r}
check_solution(c(E[21], I[21]), 1, 1, rE, rI, signal = "test")
```
...
I_star <- predict(polyfit(0), data.frame(t = -(seq(L) - 1) / ri))
I_l <- as.numeric(I_star) / ri

EI_states <- c(E_k, I_l)

check_solution(EI_states, K, L, rE, rI)
...
check_solution(EI_states, K, L, re, ri, signal = "prevalence")
...
## Initialising directly via solving the ODE
...
### Implementing for the test signal
```{r}
K <- 2 # Number of E states
L <- 2
ri <- 1
...
times <- seq(- K / re, 0, length.out = 10)
ss <- predict(polyfit(0), data.frame(t = times))
plot(t, II)
lines(t0 + times, ss, col = "red")
...
if (L > 1) {
  I_l <- predict(polyfit(0), data.frame(t = -(seq(2, L) - 1) / ri - K / re)) |> as.numeric()  
...
m <- DiseasyModelOdeSeir$new(
  ...
  variant = DiseasyVariant$new(n_variants = 1),
  compartment_structure = c("E" = K, "I" = L, "R" = 1),
  disease_progression_rates = c("E" = re / K, "I" = ri / L),
  parameters = list("age_cuts_lower" = 0, "overall_infection_risk" = 0)
)
