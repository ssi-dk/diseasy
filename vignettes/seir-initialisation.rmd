...
synthetic_model <- combinations |>
  purrr::pmap(\(K, L) {
    ...
    deSolve::ode(y = y0, times = seq(0, 60), func = private %.% rhs, parms = NULL, atol = 1e-12, rtol = 1e-12)
  })
...
incidence_polyfits <- purrr::map(incidence_signal, ~ {
  lm(incidence ~ poly(t, poly_order, raw = TRUE), data = dplyr::filter(dplyr::mutate(.x, t = t - t0), t <= 0))
})
...
```{r}
simplified_model_fit_gg <- purrr::map(names(signal_derivatives), ~ {
  n_states <- as.numeric(stringr::str_extract_all(.x, r"{\d}")[[1]])
  K <- n_states[1]
  L <- n_states[2]
  ...
  times <- seq(- 1 / rE, 0, length.out = 10)
  ss <- predict(polyfits[[.x]], data.frame(t = times))
  ...
  if (L > 1) {
    I_l <- predict(polyfits[[.x]], data.frame(t = -(seq(2, L) - 1) / ri - K / re)) |> as.numeric()  
    EI_states <- c(EI_states, I_l)
  }
...
ggplot2::ggplot(signal_gg, ggplot2::aes(x = t, y = incidence)) +
  ggplot2::geom_line(data = exact_model_fit_gg, color = "darkgrey", linewidth = 2) +
