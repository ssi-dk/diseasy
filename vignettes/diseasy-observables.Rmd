...
# We will destroy the cache if the source code for `DiseasyObservables`
# or `diseasystore` changes
cache <- cachem::cache_disk(dir = "vignette_data/cache/diseasy-observables")
source_code_hash <- digest::digest(
  list(DiseasyObservables, packageVersion("diseasystore"))
)
...
opts <- options(
  ...
  "diseasy.DiseasyObservables.cache" = cache
)
...
tmp_dir <- stringr::str_replace_all(
  tempdir(),
  stringr::fixed(r"{\\}"),
  .Platform$file.sep
)
