...
test_that("forcing functions can be configured as expected (SIR single variant / single age group)", {
  ...
  m$set_forcing_functions(state_vector_forcing = \(t, dy_dt, loss_due_to_infections, new_infections) t)
  expect_identical(private$state_vector_forcing, \(t, dy_dt, loss_due_to_infections, new_infections) t)
