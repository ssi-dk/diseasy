...
test_that("active binding: variants works", {
  ...
  expect_identical(
    tryCatch(var$variants <- list("name" = "WT"), error = \(e) e),                                       # nolint: implicit_assignment_linter
...
test_that("active binding: cross_immunity works", {
  ...
  # test_that cannot capture this error, so we have to hack it
  expect_identical(
    tryCatch(var$cross_immunity <- matrix(2), error = \(e) e),                                           # nolint: implicit_assignment_linter
